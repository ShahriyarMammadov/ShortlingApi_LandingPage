  ...
  const handleonClick = async () => {
    if (state.value?.length === 0 || state.value.trim() === "") {
      ...
      const Data = await axios.get(
        `https://api.shrtco.de/v2/shorten?url=${state.value}`
      );
      setLoading(false);
            ...
            {arr.map((e, i) => {
              return (
                <div className="urlBoxs" key={i}>
                  ...
                  <div className="right">
                    <div className="link">
                      <a href={e.full_short_link3} target="_blank">
                        {e.full_short_link3}
                      </a>
                    </div>
                    <button
                      onClick={() => handleCopy(e, i)}
                      className={state.copied === i ? "copiedBtn" : null}
                    >
